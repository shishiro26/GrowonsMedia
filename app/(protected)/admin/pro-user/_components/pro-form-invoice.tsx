...
import { acceptProInvoice, rejectProInvoice } from "@/actions/user-pro-money";
...
const ProFormInvoice = ({ id, userId }: InvoiceProps) => {
  ...
  const handleAccept = async () => {
    ...
    startTransition(() => {
      acceptProInvoice(values).then((data) => {
        if (data?.success) {
  ...
  const handleReject = async (values: z.infer<typeof RejectInvoiceSchema>) => {
    startTransition(() => {
      rejectProInvoice({ id: id, reason: values.reason }).then((data) => {
        if (data?.success) {
          toast.success(data.success, {
            action: {
              label: "close",
